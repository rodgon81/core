...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    def async_add_alarm_entity(config: dict):
        ...
        alarm_entity = AlarmoAreaEntity(hass=hass, name=config[const.ATTR_NAME], area_id=config["area_id"], coordinator=coordinator, entry_id=entry.entry_id)
        hass.data[const.DOMAIN][entry.entry_id][const.DATA_AREAS][config["area_id"]] = alarm_entity
    ...
    def async_add_alarm_master(config: dict):
        """Add each entity as Alarm Control Panel."""
        alarm_entity = AlarmoMasterEntity(hass=hass, name=config[const.CONF_HIK_NAME], coordinator=coordinator, entry_id=entry.entry_id)
        hass.data[const.DOMAIN][entry.entry_id][const.DATA_MASTER] = alarm_entity
        async_add_entities([alarm_entity])
...
class AlarmoBaseEntity(HikAlarmEntity, AlarmControlPanelEntity, RestoreEntity):
    ...
    def code_format(self):
        ...
        if self._state == const.STATE_ALARM_DISARMED and self.code_arm_required:
            return self._config[const.CONF_HIK_CODE_FORMAT]

        elif self._state != const.STATE_ALARM_DISARMED and self._config[const.CONF_HIK_CODE_DISARM_REQUIRED]:
            return self._config[const.CONF_HIK_CODE_FORMAT]
    ...
    def code_arm_required(self):
        ...
        return self._config[const.CONF_HIK_CODE_ARM_REQUIRED]

    @property
    def arm_mode(self):
        """Return the arm mode."""
        return self._arm_mode if self._state != const.STATE_ALARM_DISARMED else None
    ...
    def _validate_code(self, code, state):
        ...
        if state == const.STATE_ALARM_DISARMED and not self._config[const.CONF_HIK_CODE_DISARM_REQUIRED]:
            return (True, None)
        elif state != const.STATE_ALARM_DISARMED and not self._config[const.CONF_HIK_CODE_ARM_REQUIRED]:
            return (True, None)
        ...
        if self._config[const.CONF_HIK_CODE]:
            hash = base64.b64decode(self._config[const.CONF_HIK_CODE])
        ...
        elif self._config[const.CONF_HIK_AREA_LIMIT] and not all(
            area in self._config[const.CONF_HIK_AREA_LIMIT] for area in ([self.area_id] if self.area_id else list(self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].keys()))
        ):
            # user is not allowed to operate this area
            _LOGGER.debug("User {} has no permission to arm/disarm this area.".format(self._config[const.CONF_HIK_USERNAME]))
            return (False, const.EVENT_INVALID_CODE_PROVIDED)
        elif state == const.STATE_ALARM_DISARMED and not self._config[const.CONF_HIK_CAN_DISARM]:
            # user is not allowed to disarm the alarm
            _LOGGER.debug("User {} has no permission to disarm the alarm.".format(self._config[const.CONF_HIK_USERNAME]))
            return (False, const.EVENT_INVALID_CODE_PROVIDED)
        elif state in const.ARM_MODES and not self._config[const.CONF_HIK_CAN_ARM]:
            # user is not allowed to arm the alarm
            _LOGGER.debug("User {} has no permission to arm the alarm.".format(self._config[const.CONF_HIK_USERNAME]))
            return (False, const.EVENT_INVALID_CODE_PROVIDED)
        else:
            return (True, None)
    ...
    async def async_alarm_disarm(self, **kwargs):
        ...
        if self._state == const.STATE_ALARM_DISARMED or not self._config:
            if not self._config:
                _LOGGER.warning("Cannot process disarm command, alarm is not initialized yet.")
            else:
                _LOGGER.warning("Cannot go to state {} from state {}.".format(const.STATE_ALARM_DISARMED, self._state))

            async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_COMMAND_NOT_ALLOWED, self.area_id, {"state": self._state, "command": const.COMMAND_DISARM, const.ATTR_CONTEXT_ID: context_id})

            return

        (res, info) = self._validate_code(code, const.STATE_ALARM_DISARMED)
        ...
        if not res and not skip_code:
            ...
            await self.async_update_state(const.STATE_ALARM_DISARMED)

            _LOGGER.info("Alarm '{}' is disarmed.".format(self.name))

            async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_DISARM, self.area_id, {const.ATTR_CONTEXT_ID: context_id})
    ...
    async def async_service_arm_handler(self, code, mode, skip_delay, force, context_id=None):
        ...
        await self.async_handle_arm_request(mode, code=code, skip_delay=skip_delay, bypass_open_sensors=force, context_id=context_id)
    ...
    async def async_handle_arm_request(self, arm_mode, **kwargs):
        ...
        if not (const.MODES_TO_SUPPORTED_FEATURES[arm_mode] & self.supported_features) or (self._state != const.STATE_ALARM_DISARMED and self._state not in const.ARM_MODES) or not self._config:
            ...
            async_dispatcher_send(
                self.hass, "alarmo_event", const.EVENT_COMMAND_NOT_ALLOWED, self.area_id, {"state": self._state, "command": arm_mode.replace("armed", "arm"), const.ATTR_CONTEXT_ID: context_id}
            )
        ...
        if self._state in const.ARM_MODES:
            ...
            self._revert_state = const.STATE_ALARM_DISARMED
            self.open_sensors = None
            self.bypassed_sensors = None

        await self.async_arm(arm_mode, skip_delay=skip_delay, bypass_open_sensors=bypass_open_sensors, context_id=context_id)

    async def async_alarm_arm_away(self, code=None, skip_code=False):
        """Send arm away command."""
        _LOGGER.debug("alarm_arm_away")
        await self.async_handle_arm_request(const.STATE_ALARM_ARMED_AWAY, code=code, skip_code=skip_code)

    async def async_alarm_arm_home(self, code=None, skip_code=False):
        """Send arm home command."""
        _LOGGER.debug("alarm_arm_home")
        await self.async_handle_arm_request(const.STATE_ALARM_ARMED_HOME, code=code, skip_code=skip_code)

    async def async_alarm_arm_night(self, code=None, skip_code=False):
        """Send arm night command."""
        _LOGGER.debug("alarm_arm_night")
        await self.async_handle_arm_request(const.STATE_ALARM_ARMED_NIGHT, code=code, skip_code=skip_code)
    ...
    async def async_alarm_arm_custom_bypass(self, code=None, skip_code=False):
        ...
        await self.async_handle_arm_request(const.STATE_ALARM_ARMED_CUSTOM_BYPASS, code=code, skip_code=skip_code)

    async def async_alarm_arm_vacation(self, code=None, skip_code=False):
        """Send arm vacation command."""
        _LOGGER.debug("alarm_arm_vacation")
        await self.async_handle_arm_request(const.STATE_ALARM_ARMED_VACATION, code=code, skip_code=skip_code)
...
class AlarmoMasterEntity(AlarmoBaseEntity):
    ...
    def __init__(self, hass: HomeAssistant, name: str, coordinator: HikAlarmDataUpdateCoordinator, entry_id) -> None:
        ...
        self._config = self.coordinator.store.async_get_alarm_config()

    @property
    def supported_features(self) -> int:
        """Return the list of supported features."""

        supported_features = [item.supported_features or 0 for item in self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].values()]
    ...
    async def async_added_to_hass(self):
        ...
        async def async_update_config(area_id=None):
            if area_id and area_id in self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS]:
        ...
        self.async_on_remove(async_dispatcher_connect(self.hass, "alarmo_config_updated", async_update_config))
        ...
        async def async_handle_event(event: str, area_id: str, args: dict = {}):
            if not area_id or event not in [const.EVENT_FAILED_TO_ARM, const.EVENT_TRIGGER, const.EVENT_TRIGGER_TIME_EXPIRED]:
                return
            ...
            if event == const.EVENT_TRIGGER and (
                self._state not in [const.STATE_ALARM_TRIGGERED, const.STATE_ALARM_PENDING] or (self._state == const.STATE_ALARM_PENDING and self.delay and self.delay > args.get("delay", 0))
            ):
                # only pass initial trigger event or while trigger with shorter entry delay occurs during entry time
                async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_TRIGGER, self.area_id, args)
            if event == const.EVENT_TRIGGER_TIME_EXPIRED:
                if self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS][area_id].state == const.STATE_ALARM_DISARMED:
    ...
    async def async_update_state(self, state: str = None):
        ...
        states = [item.state for item in self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].values()]

        state = None

        if const.STATE_ALARM_TRIGGERED in states:
            state = const.STATE_ALARM_TRIGGERED
        elif const.STATE_ALARM_PENDING in states:
            state = const.STATE_ALARM_PENDING
        elif const.STATE_ALARM_ARMING in states and all(el in const.ARM_MODES or el == const.STATE_ALARM_ARMING for el in states):
            state = const.STATE_ALARM_ARMING
        elif all(el == const.STATE_ALARM_ARMED_AWAY for el in states):
            state = const.STATE_ALARM_ARMED_AWAY
        elif all(el == const.STATE_ALARM_ARMED_HOME for el in states):
            state = const.STATE_ALARM_ARMED_HOME
        elif all(el == const.STATE_ALARM_ARMED_NIGHT for el in states):
            state = const.STATE_ALARM_ARMED_NIGHT
        elif all(el == const.STATE_ALARM_ARMED_CUSTOM_BYPASS for el in states):
            state = const.STATE_ALARM_ARMED_CUSTOM_BYPASS
        elif all(el == const.STATE_ALARM_ARMED_VACATION for el in states):
            state = const.STATE_ALARM_ARMED_VACATION
        elif all(el == const.STATE_ALARM_DISARMED for el in states):
            state = const.STATE_ALARM_DISARMED

        arm_modes = [item._arm_mode for item in self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].values()]
        ...
        if state in [const.STATE_ALARM_ARMING, const.STATE_ALARM_PENDING]:
            # one or more areas went to arming/pending state, recalculate the delay time

            area_filter = dict(filter(lambda el: el[1].state == state, self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].items()))
            delays = [el.delay for el in area_filter.values()]

            # use maximum of all areas when arming, minimum of all areas when pending
            delay = max(delays) if state == const.STATE_ALARM_ARMING else min(delays) if len(delays) else None
        else:
            delay = None
        ...
        if arm_mode == self._arm_mode and (state == self._state or not state) and delay == self.delay and open_sensors == self.open_sensors:
    ...
    async def async_alarm_disarm(self, **kwargs):
        ...
        if res:
            for item in self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].values():
                if item.state != const.STATE_ALARM_DISARMED:
    ...
    async def async_arm(self, arm_mode, **kwargs):
        ...
        if open_sensors:
            ...
            for (area_id, entity) in self.hass.data[const.DOMAIN][self.entry_id][const.DATA_AREAS].items():
                if entity.state == const.STATE_ALARM_ARMING:
                    t = area_config[area_id][const.ATTR_MODES][arm_mode]["exit_time"]
                    delay = t if t > delay else delay

            async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_ARM, self.area_id, {"arm_mode": arm_mode, "delay": delay, const.ATTR_CONTEXT_ID: context_id})
    ...
    async def async_arm_failure(self, open_sensors: dict, context_id=None):
        ...
        self._revert_state = None
        async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_FAILED_TO_ARM, None, {"open_sensors": open_sensors, "command": command, const.ATTR_CONTEXT_ID: context_id})
        self.async_write_ha_state()
...
class AlarmoAreaEntity(AlarmoBaseEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if state:
            ...
            if initial_state == const.STATE_ALARM_ARMING:
                await self.async_arm(self.arm_mode)
            elif initial_state == const.STATE_ALARM_PENDING:
                await self.async_trigger()
            elif initial_state == const.STATE_ALARM_TRIGGERED:
                await self.async_trigger(skip_delay=True)
            else:
    ...
    async def async_update_state(self, state: str = None):
        ...
        if self.state not in [const.STATE_ALARM_ARMING, const.STATE_ALARM_PENDING]:
            self.delay = None

        if state in const.ARM_MODES:
            self._arm_mode = state
        elif old_state == const.STATE_ALARM_DISARMED and state == const.STATE_ALARM_TRIGGERED:
            self._arm_mode = None
    ...
    async def async_arm_failure(self, open_sensors: dict, context_id=None):
        ...
        async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_FAILED_TO_ARM, self.area_id, {"open_sensors": open_sensors, "command": command, const.ATTR_CONTEXT_ID: context_id})
    ...
    async def async_arm(self, arm_mode, **kwargs):
        ...
        if skip_delay or not exit_delay:
            ...
            if open_sensors:
                ...
                if self._state and self._state != const.STATE_ALARM_ARMING:
                    async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_ARM, self.area_id, {"arm_mode": arm_mode, "delay": 0, const.ATTR_CONTEXT_ID: context_id})
            ...
            if open_sensors:
                ...
                async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_ARM, self.area_id, {"arm_mode": arm_mode, "delay": exit_delay, const.ATTR_CONTEXT_ID: context_id})

                self.delay = exit_delay
                self.open_sensors = None

                await self.async_update_state(const.STATE_ALARM_ARMING)
                ...
                async def async_leave_timer_finished(now):
                    ...
                    await self.async_arm(self.arm_mode, bypass_open_sensors=bypass_open_sensors, skip_delay=True)
    ...
    async def async_trigger(self, skip_delay: bool = False, open_sensors: dict = None):
        ...
        if self._state == const.STATE_ALARM_PENDING or skip_delay or not self._arm_mode:
            entry_delay = 0
        else:
        ...
        if self._state and (self._state != const.STATE_ALARM_PENDING or (self._state == const.STATE_ALARM_PENDING and skip_delay and open_sensors != self.open_sensors)):
        ...
        if not entry_delay:
            ...
            await self.async_update_state(const.STATE_ALARM_TRIGGERED)
            ...
            if trigger_time:
                ...
                async def async_trigger_timer_finished(now):
                    ...
                    async_dispatcher_send(self.hass, "alarmo_event", const.EVENT_TRIGGER_TIME_EXPIRED, self.area_id)
            ...
            await self.async_update_state(const.STATE_ALARM_PENDING)
