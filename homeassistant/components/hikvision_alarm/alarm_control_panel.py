...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    def async_add_alarm_entity(area):
        ...
        alarm_entity = AlarmoAreaEntity(hass=hass, entity_id=entity_id, name=area.name, area_id=area.id, sub_system=area, coordinator=coordinator)
...
class AlarmoBaseEntity(CoordinatorEntity, AlarmControlPanelEntity, RestoreEntity):
    ...
    def code_format(self):
        ...
        elif self._state != STATE_ALARM_DISARMED and self._config and const.ATTR_CODE_DISARM_REQUIRED in self._config and self._config[const.ATTR_CODE_DISARM_REQUIRED]:
            return self._config[ATTR_CODE_FORMAT]
    ...
    def _validate_code(self, code, state):
        ...
        elif res[const.ATTR_AREA_LIMIT] and not all(area in res[const.ATTR_AREA_LIMIT] for area in ([self.area_id] if self.area_id else list(self.hass.data[const.DOMAIN]["areas"].keys()))):
    ...
    async def async_handle_arm_request(self, arm_mode, **kwargs):
        ...
        if not (const.MODES_TO_SUPPORTED_FEATURES[arm_mode] & self.supported_features) or (self._state != STATE_ALARM_DISARMED and self._state not in const.ARM_MODES) or not self._config:
...
class AlarmoMasterEntity(AlarmoBaseEntity):
    ...
    def supported_features(self) -> int:
        ...
        supported_features = [item.supported_features or 0 for item in self.hass.data[const.DOMAIN]["areas"].values()]
    ...
    async def async_added_to_hass(self):
        ...
        async def async_handle_event(event: str, area_id: str, args: dict = {}):
            ...
            if event == const.EVENT_TRIGGER and (
                self._state not in [STATE_ALARM_TRIGGERED, STATE_ALARM_PENDING] or (self._state == STATE_ALARM_PENDING and self.delay and self.delay > args.get("delay", 0))
            ):
    ...
    async def async_update_state(self, state: str = None):
        ...
        arm_modes = [item._arm_mode for item in self.hass.data[const.DOMAIN]["areas"].values()]
...
class AlarmoAreaEntity(AlarmoBaseEntity):
    ...
    def __init__(self, hass: HomeAssistant, name: str, entity_id: str, area_id: str, sub_system, coordinator) -> None:
        """Initialize the alarm_control_panel entity."""
        super().__init__(hass, name, entity_id, coordinator)
    ...
    async def async_trigger(self, skip_delay: bool = False, open_sensors: dict = None):
        ...
        if self._state and (self._state != STATE_ALARM_PENDING or (self._state == STATE_ALARM_PENDING and skip_delay and open_sensors != self.open_sensors)):
