...
def websocket_get_alarm_entities(hass, connection, msg):
    ...
    result = [
        ...
        for (area_id, entity) in hass.data[const.DOMAIN][const.DATA_AREAS].items()
    ]

    if hass.data[const.DOMAIN][const.DATA_MASTER]:
        result.append({
            "entity_id": hass.data[const.DOMAIN][const.DATA_MASTER].entity_id,
            "area_id": 0
        })
...
def websocket_get_countdown(hass, connection, msg):
    ...
    item = next((entity for entity in hass.data[const.DOMAIN][const.DATA_AREAS].values(
    ) if entity.entity_id == entity_id), None)
...
async def async_register_websockets(hass):
    async_register_command(
        ...
        websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
            {vol.Required("type"): "hikvision_alarm/config"}
        ),
    )
    async_register_command(
        hass,
        ...
        websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
            {vol.Required("type"): "hikvision_alarm/entities"}
        ),
    )
    async_register_command(
        hass,
        ...
        websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend(
            {
                vol.Required("type"): "hikvision_alarm/countdown",
